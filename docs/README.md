...
| `aspectRatio`          | `string` / `object` | Reserves space for each slide's content using a CSS [`aspect-ratio`](https://developer.mozilla.org/en-US/docs/Web/CSS/aspect-ratio) (e.g. `"16/9"`), preventing layout shift (CLS) while the content loads. This is a responsive prop (object with `desktop`, `tablet`, and `phone`). Optional: when left unset, no CSS is applied and behavior is unchanged. Takes precedence over `minHeight` when both are set.                                                                                                                                                                                            | `undefined`                            |
| `minHeight`            | `number` / `object` | Reserves a minimum height, in pixels, for each slide's content, preventing layout shift (CLS) while the content loads. This is a responsive prop (object with `desktop`, `tablet`, and `phone`). Optional: when left unset, no CSS is applied and behavior is unchanged. Ignored when `aspectRatio` is set.                                                                                                                                                                                                                                                                                                     | `undefined`                            |
